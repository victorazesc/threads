        ...
        <TooltipTrigger asChild disabled={!showTooltip}>
          <div
            className={`${!isAValidNumber(size) ? sizeInfo.avatarSize : ""
              } grid place-items-center overflow-hidden relative ${getBorderRadius(shape)}`}
            style={
              isAValidNumber(size)
                ? {
                  height: `${size}px`,
            ...
            {src ? (
              ...
              <div
                className={`${!isAValidNumber(sizeInfo.fontSize) ? sizeInfo.fontSize : ""} grid h-full w-full place-items-center ${getBorderRadius(
                  shape
                )} ${className}  ${fallbackBackgroundColor}`}
                style={{
                  // backgroundColor: fallbackBackgroundColor ?? "rgba(var(--barcelona-tertiary-background))",
